...
1. Open `Clueless.dmg` and drag **Clueless** into **Applications**.
...
make app    # build dist/Clueless.app
make run    # build and launch
make dmg    # package dist/Clueless.dmg
```
...
- **Consent:** you are transcribing other people. Recording/transcription laws vary by jurisdiction (some require all-party consent) — tell your participants and check your local rules before using Clueless in real meetings.
...
```
Sources/
  CluelessCore/      # pure logic: parsers, stores, prompt builder, LLM providers
  Clueless/          # macOS app: SwiftUI overlay/UI, audio capture, services
Tests/
  CluelessCoreTests/ # swift-testing suites for everything in CluelessCore
```

- `CluelessCore` has no AppKit dependencies and is fully unit-tested (`make test`).
